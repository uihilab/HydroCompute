...
export default class webgpu{
    ...
    static async deviceCall(){
        ...
        if (!this.adapter) {
            console.error('This function cannot be used, WebGPU not available in your browser'); return Error()
        }
    }
    ...
    static async digestData(...args) {
        ...
        args[1].sizes = (()=>{
            if ((args[0][0].length % Math.sqrt(args[0][0].length) === 0) 
            && (args[0][1].length % Math.sqrt(args[0][1].length) === 0)) {
                return [[Math.sqrt(args[0][0].length),Math.sqrt(args[0][0].length)],[Math.sqrt(args[0][1].length), Math.sqrt(args[0][0].length)]]
            } else {
        ...
        [rSize, rBuffer] = buffers.resultHolder(this.device, [mat1, mat2], args[1].function),
        ...
        bindgroup = bind.bindGroup(this.device, bindGroupLayout, [group1, group2, group3]),

        shader = shade.shaderModule(this.device, 
            args[1].function === "matrixMul" ? matrixUtils.matrixMul() : matrixUtils.matrixAdd()
        ),
